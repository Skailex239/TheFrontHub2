...
//      présence) puis `active: boolean.optional()` (2 bits : présence+valeur)
//      → le header de présence passe de 0 octet à 1 OCTET. L'ancien décodeur
//      (v5.14) lisait ce byte de header comme début de serverTime → tout le
//      flux se désalignait (« implausible count 1789… » = timestamp lu comme
//      compteur de games). Frames counts inchangées (0 bit → 0 octet).
//      Source : OpenFrontIO v0.34.0 src/core/Schemas.ts — le commentaire
//      amont dit explicitement qu'un bundle antérieur ne peut plus décoder.
//   2. GameConfig REGAGNE `trusted: boolean.optional()` (2 bits), entre
//      allowedPublicIds et maxTimerValue — exactement la position que v5.13
//      avait ajoutée et que v5.14 avait retirée.
//   3. GameMapType : 6 nouvelles maps (Cape Cod, Central America, Channel
//      Islands, Gulf Of Mexico, Qing China, Yangtze River) — toutes en
//      insertion ordre-préservée. La table v5.14 avait en plus Yangtze River
//      au MAUVAIS ordinal (115 au lieu de 120, réintroduite en v0.34) → les
//      ordinaux 115-116 décodaient des noms faux depuis le 04/09. Table
//      reconstruite à l'identique de Maps.gen.ts v0.34.0 (123 entrées).
//   Vérifié contre le WS live wss://blue.openfront.io/w0/lobbies le 14/09 :
//   avant patch = 19 counts OK / 0 full décodé ; après patch = full décodés.
...
(function (global) {
  ...
  const GameConfig = obj([
    ...
    // ⚠️ v5.15 (v0.34.0) : `trusted` REGAGNE sa place (v5.13 → v5.14 → v5.15 !)
    // entre allowedPublicIds et maxTimerValue. 2 bits (présence + valeur).
  ...
  const PublicLobbyFull = obj([
    ...
    // ⚠️ v5.15 (v0.34.0) : ces 2 champs optionnels font passer le header de
    // présence de 0 octet à 1 octet (gitCommit = 1 bit, active = 2 bits).
    // C'est CE changement qui cassait le décodage des frames full en v5.14.
