...
// Expose Firestore DB + current user for non-module scripts (e.g. lobby.js)
window._firestoreDb = db;
window._firestoreAuth = auth;
window._authUser = null;
window._onAuthUserChanged = [];

// Subscribe to auth state changes and propagate to window._authUser + listeners
onAuthStateChanged(auth, (user) => {
  window._authUser = user;
  for (const cb of window._onAuthUserChanged) {
    try { cb(user); } catch (e) { console.warn("[auth] _onAuthUserChanged callback error:", e); }
  }
});

// Also expose Firestore helpers globally so non-module scripts can use them
window._firestore = {
  doc, getDoc, getDocs, setDoc, updateDoc,
  collection, query, where, onSnapshot,
  increment, deleteField, runTransaction,
